...
COPY src/ /temp/
RUN cmd.exe Invoke-Expression 'robocopy C:/temp C:/nuget/src /s /ndl /njh /njs *.csproj *.scproj packages.config'
...
RUN cmd.exe curl -sSL "https://dist.nuget.org/win-x86-commandline/v5.8.1/nuget.exe" -o "%ProgramFiles%\NuGet\nuget.exe"


# DEMO TEAM CUSTOMIZATION - Needed?
# Install latest PackageProvider (required for Sitecore.Courier)
RUN cmd.exe Install-PackageProvider -Name NuGet -MinimumVersion 2.8.5.201 -Force
...
RUN cmd.exe nuget restore -Verbosity quiet

# Copy remaining source code
COPY src/ ./src/

# Build the Sitecore main platform artifacts
RUN cmd.exe msbuild .\src\platform\Platform.csproj /p:Configuration=$env:BUILD_CONFIGURATION /m /p:DeployOnBuild=true /p:PublishProfile=Local

# Save the artifacts for copying into other images (see 'cm' and 'rendering' Dockerfiles).
FROM mcr.microsoft.com/windows/nanoserver:1809
WORKDIR /artifacts

COPY ./docker/deploy/platform  ./sitecore/
